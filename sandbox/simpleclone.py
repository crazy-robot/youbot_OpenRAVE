...
class SimpleNavigationPlanning:
	...
	t.SetTransform(g)
        while True:
            ...
            traj = self.basemanip.MoveActiveJoints(goal=goal,maxiter=3000,steplength=0.1,outputtrajobj=True,execute=False)
	    print traj
	    #trajdata = trajobj.Sample(1)
	    ...
	    for i in range(5):
   		...
   		while time.time()-starttime < traj.GetDuration():
        		...
        		with self.env: # have to lock environment since accessing robot
            			...
            			values=spec.ExtractJointValues(trajdata,self.robot,range(self.robot.GetActiveDOF()),0)
                                #values = spec.ExtractAffineValues(trajdata,range(self.robot.GetActiveDOF()),0)
				values = spec.ExtractAffineValues(trajdata,DOFAffine.X|DOFAffine.Y|DOFAffine.RotationAxis)
				#print values
...
def main(env,options):
    ...
    time.sleep(5) # give time for environment to update
